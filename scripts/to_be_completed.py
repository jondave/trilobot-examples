# This script is an uncompleted version of "color_based_operation.py" which uses the camera mounted on the trilobot to detect, locate and 
# determine the color of circles/balls in the image. 
...
def action_planner(color,x_pos,width):
    ...
    # if color="GREEN" then robot_action="MOVING FORWARD FOR 3 SECONDS"
    # if color="YELLOW then robot_action="FOLLOWING A SQUARE PATH"
    # if color="BLUE" then robot_action="FOLLOWING A CIRCULAR PATH"
    # if color="RED" then robot_action="TRACKING THE BALL", this will require to use variables "width" and "x_pos" 
    # Remember to activate the LEDs with the corresponding color while the robot is performing an action. There is
    # a function called activate_leds() defined below that can be used for this.
